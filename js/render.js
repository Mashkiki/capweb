...
function renderCrystal(crystal) {
  ...
  let crystalImage = document.createElement("div")
  crystalImage.className = "crystal-image"
  crystalImage.style = `position: absolute; width: 100%; height: 100%;`
  crystalImage.style.background = fillStyle
  crystalImage.style.backgroundSize = "contain"
  crystalImage.style.backgroundRepeat = "no-repeat"
  crystalElement.appendChild(crystalImage)
  ...
  crystalImage.style.filter = `${areaCrystalFilter[crystal.area]} drop-shadow(3px 3px 0px #00000080)`
  growAnimation(crystal, crystalImage)
...
function formatNumber(n) {
  let numberSuffixes = ["", "k", "m", "b", "t", "qd", "qn", "sx"]
  if (n < 10_000_000_000) {
    return n.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",");
  }
  if (n >= 10_000_000_000 && n < 10^33) {
    let temp = Math.round(n).toString()
    let digits = temp.length
    let thousandExponent = Math.floor((digits - 1) / 3)
    let pointPosition = digits - (thousandExponent * 3)
    let firstDigits = temp.substring(0, pointPosition)
    let secondDigits = temp.substring(pointPosition, pointPosition + (4 - pointPosition))
    return (firstDigits + '.' + secondDigits + numberSuffixes[thousandExponent])
  }
}
